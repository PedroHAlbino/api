...
func (repositorio Usuarios) Seguir(usuarioID, seguidorID uint64) error {
	statement, erro := repositorio.db.Prepare(
		"insert ignore into seguidores (usuarios_id, seguidor_id) values (?,?)",
	)
	if erro != nil {
		return erro
	}
...
func (repositorio Usuarios) ParaDeSeguir(usuarioID, seguidorID uint64) error {
	statement, erro := repositorio.db.Prepare(
		"delete from seguidores where usuarios_id = ? and seguidor_id = ?",
	)
	if erro != nil {
		return erro
	}
...
func (repositorio Usuarios) BuscarSeguidores(usuarioID uint64) ([]models.Usuario, error) {
	linhas, erro := repositorio.db.Query(`
		...
		from usuarios u inner join seguidores s on u.id = s.seguidor_id where s.usuarios_id = ?
	`, usuarioID)
